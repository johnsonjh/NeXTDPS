...
#ifdef NeXT_MOD
    if (PrinterName == NULL
	    && (PrinterName = (char *)NXGetDefaultValue( "System",
							"Printer")) == NULL
	    && (PrinterName = envget("PRINTER")) == NULL)
		PrinterName = POSTSCRIPTPRINTER;
#else
...
#ifdef NeXT_MOD
    if (signal(SIGINT, (int (*)())int1) == SIG_IGN)
#else
...
#ifdef NeXT_MOD
	VOIDC signal(SIGQUIT, (int (*)())int1);
	VOIDC signal(SIGHUP, (int (*)())int1);
	VOIDC signal(SIGTERM, (int (*)())int1);
#else
