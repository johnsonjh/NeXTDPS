...
_NOTICE RCSID[]="$Header: pscomm.bsd,v 2.2 87/11/17 16:51:17 byron Rel $";
#endif
/* pscomm.c
 *
...
#ifdef NeXT_MOD
    /* signal.h has signal taking int funcs as args */
    VOIDC signal(SIGINT, (int (*)())GotDieSig);
    VOIDC signal(SIGHUP, (int (*)())GotDieSig);
    VOIDC signal(SIGTERM, (int (*)())GotDieSig);
    VOIDC signal(SIGALRM, (int (*)())GotAlarmSig);
    VOIDC signal(SIGEMT, (int (*)())GotEmtSig);
#else
...
#if	NeXT
    /*
     * This is a workaround to a kernel problem.  Turn down the
     * interrupt delay on the serial line so that flow control will
     * work fast enough.
     */
    if (isatty(fdsend)) {
	int	usec = 0;

	/* We don't care if the ioctl fails, things will still work */
	(void) ioctl(fdsend, ZIOCTSET, &usec);
    }
#endif	NeXT
