...
#ifdef NeXT_DEBUG
	syslog( LOG_ERR, "printjob() on %s", printer );
#endif
...
#ifdef NeXT_DEBUG
		syslog( LOG_ERR, "Printing disabled" );
#endif
		close( lfd );
...
#ifdef NeXT_DEBUG
		syslog( LOG_ERR, "Active daemon present, exiting" );
#endif
		  exit(0);
		}
		syslog(LOG_ERR, "%s: %s: %m", printer, LO);
...
#ifdef NeXT_DEBUG
	syslog( LOG_ERR, "printjob(), calling openpr()" );
#endif
	openpr();			/* open printer or remote */

#ifdef NeXT_DEBUG
	syslog( LOG_ERR, "printjob(), called openpr()" );
	syslog( LOG_ERR, "nitems %d", nitems );
#endif
again:
	/*
	...
	for (qp = queue; nitems--; free((char *) q)) {
		q = *qp++;
#ifdef NeXT_DEBUG
	syslog( LOG_ERR, "printing queue 0x%08x", q );
	syslog( LOG_ERR, "printing %s", q->q_name );
#endif
		if (stat(q->q_name, &stb) < 0){
#ifdef NeXT_DEBUG
        syslog( LOG_ERR, "stat of file failed" );
#endif			
			continue;
		}
	restart:
		...
		if (!remote){
#ifdef NeXT_DEBUG
		  syslog( LOG_ERR, "calling printit() with %s\n", q->q_name );
#endif
			i = printit(q->q_name);
		}
		else{
#ifdef NeXT_DEBUG
		  syslog( LOG_ERR, "calling sendit() with %s", q->q_name );
#endif
			i = sendit(q->q_name);
		}
#ifdef NeXT_DEBUG
		  syslog( LOG_ERR, "returned with %d", i );
#endif
		...
		if (fstat(lfd, &stb) == 0) {
			...
			if (stb.st_mode & 0100){
			  syslog( LOG_ERR, "stop bit found on lockfile\n" );
				goto done;
			}
			/* rebuild queue (after lpc topq) */
			if (stb.st_mode & 01) {
			  syslog( LOG_ERR, "rebuild bit found on lockfile\n" );
...
#ifdef NeXT_DEBUG
	  syslog( LOG_ERR, "printjob loop done" );
#endif
	...
	if (nitems == 0) {		/* no more work to do */
		...
		(void) unlink(tmpfile);
#ifdef NeXT_DEBUG
		syslog( LOG_ERR, "printjob done on %s", printer );
#endif
		exit(0);
	}
	goto again;
	...
	char *file;
{
	...
	if ((stb.st_mode & S_IFMT) == S_IFLNK && fstat(fi, &stb) == 0 ){
	  ...
	  if( stb.st_ino != fino )
	    return(ACCESS);
#else    
	  if( (stb.st_dev != fdev || stb.st_ino != fino) )
	    return(ACCESS);
#endif
	}
...
start:
	...
	if ((child = dofork(DORETURN)) == 0) {	/* child */
		dup2(fi, 0);
		dup2(fo, 1);
		/* The code below stuffs the errors reported by the filter into a temp file.
		   This file is supposed to be mailed to users on errors.  (1) It is never
		   mailed.  Problems with return codes.  (2) I am suspicious about how easy
		   it is to get back to a user.  Should the address include the host?  If so,
		   what host?  Etc...  Also, the errors should STILL be reported in the log,
		   so the printer administrator can find out about them.  A better fix might
		   be to append the temp file to the log file.  Commenting this code out
		   has the filter append to the log file directly.  */
#ifndef NeXT_MOD
	...
	tof = 0;
	if (!WIFEXITED(status)) {
		syslog(LOG_WARNING, "%s: filter %s terminated (%d), (%d)",
			printer, prog, status.w_termsig, status.w_retcode);
		return(ERROR);
	}
...
abortpr()
{
	...
	while (wait(0) > 0)
		;
#ifdef NeXT_DEBUG
        syslog( LOG_ERR, "abort: done waiting" );
#endif
#ifdef NeXT_MOD
	/* For some reason on our system, startdaemon's are executing early enough that the
	 * flock on the lock file from the *aborted* lpd is still around, which prevents the
	 * new lpd from starting up.  Maybe close takes longer on our system than on others
	 * for some reason?  Anyway, just putting startdaemon here didn't fix the problem.
	 * The close operation is needed as well.
	 */
	close(lfd);
	startdaemon(printer);
#endif
...
#ifdef NeXT_MOD
		/* This code avoids a loop when the local printer entry of a public printer
		   gets smashed (like when somebody with a public printer does a BuildDisk).
		   Otherwise, jobs get to the host of the public printer, then lpd on that
		   host loops forwarding the jobs to that public printer, since there is no
		   local printer entry.  NOTE: We want to allow forwarding to a different
		   printer on the same host, so public names for printers can be different
		   from the local ones.  */
		if (strcmp(RM,host) == 0 && strcmp(RP,printer) == 0) {
			status("There is no local printer called %s on %s", printer, host);
			syslog(LOG_ERR, "%s: No local printer on host", printer);
			exit(1);
		}
#endif
