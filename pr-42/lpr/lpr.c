...
NXDefaultsVector printerDefaults = {
  { "Printer", "Local_Printer" },
  { "SystemAlert", "Panel" },
  { "PrinterResolution", "400 dpi" },
  { 0, 0 }
};
extern int NXArgc;
	...
	char *argv[];
{
	...
	NXRegisterDefaults( "System", printerDefaults );
	argc = NXArgc;
	...
	*resolution = 0;
...
#ifdef NeXT_MOD
		case 'R':		/* specifiy resolution */
			if (arg[2])
				strcpy( resolution, &arg[2] );
			else if (argc > 1) {
				argc--;
				strcpy( resolution, *++argv );
			}
			break;

		case 'M':		/* specifiy manualfeed */
			manualfeed = 1;
			break;
#endif
...
#ifdef NeXT_MOD
	if (printer == NULL
	    && (printer = (char *)NXGetDefaultValue( "System", "Printer")) == NULL
...
#endif
#ifdef NeXT_DEBUG
	syslog (LOG_ERR, "Lpd: Printer %s", printer);
#endif NeXT_DEBUG
	...
	(void) fchown(tfd, DU, -1);	/* owned by daemon for protection */
	  ...
	  card( 'F', "" );
	if (*resolution == '\0') {
	  const char *defaultRes = NXGetDefaultValue("System", "PrinterResolution");
	  if (strcmp(defaultRes, "300 dpi") == 0) strcpy(resolution, "300");
	  if (strcmp(defaultRes, "400 dpi") == 0) strcpy(resolution, "400");
	}
	if (*resolution)
	  card('R', resolution);
#endif
	...
	char *n;
{
	...
	(void) umask(oldumask);
	if (f < 0) {
		printf("%s: cannot create %s, error %d", name, n, errno);
		cleanup();
	}
...
#ifdef NeXT_MOD
	if (read(fd, &machh, sizeof(machh)) == sizeof(machh))
		switch(machh.magic) {
		case MH_MAGIC:
