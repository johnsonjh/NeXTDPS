...
#ifdef NeXT_MOD
extern short    RE;             /* printer resolution */
extern short    MF;             /*  printer manual feed */
#endif
...
struct queue {
	...
	char    q_owner[MAXNAMLEN+1];     /* owner of the job */
	char    q_fileName[MAXNAMLEN+1]; /* file name */
	char    q_docName[MAXNAMLEN+1];  /* document name */
	char    q_creator[MAXNAMLEN+1];
	int     q_size;                   /* byte size of the document */
	int     q_pages;                  /* number of pages in the file */
	int     q_manualFeed;            /* manual feed flag */
};
