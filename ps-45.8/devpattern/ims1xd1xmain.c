...
Paul Rovner: Fri Dec 29 11:10:16 1989
Jim Sandman: Tue Aug  8 17:04:07 1989
  ...
  switch (log2BPS) {
    case 1: {
      Set2BitValues(
        ...
        &im->source->decode[IMG_GRAY_SAMPLES]);
      break;
      }
    case 2: {
      Set4BitValues(
	...
	if (IntPart(fxdSrcX) == IntPart(endX)) { /* vertical source */
	  ...
	  if (sv.y > 0) {
		...
		do { /* pixelX loop */
		  ...
		  if (pixThresholds[sData] < *screenElt++)
		    temp |= SHIFTPIXEL(g,b);
