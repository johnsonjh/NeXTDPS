...
static const void (*(moveLineDispatchSW[2][6][4]))() = {
 {{WCOPYConLine, RWCOPYBmULine, WCOPYPatLine, RWCOPYBmALine },
...
void MRInitialize()
{
    ...
    moverectlen = MPSCREEN_ROWWORDS;
    moverectbuff = (uint *)MNew(moverectlen, sizeof(uint));
    if (use_wf_hardware)
		...
		int height, int argNumInts, int numLastPix, int lToR)
{
	...
	case 3:
		...
		case FIRSTF1OP + 0:	/* D = (1-D)*S */
		    sI[1] = dI;
		    if (use_wf_hardware) { BUFFERSOURCEHW(2, WCOPY); }
		    else BUFFERSOURCE(2, WCOPY);
		    break;
		case FIRSTF2OP + 6:	/* D = D + S0*(1-S1) */
		    if (use_wf_hardware) { BUFFERSOURCEHW(2, WF1); }
		    else BUFFERSOURCE(2, WF1);
		    break;
		default:
				CantHappen();
	    } /* switch (op) */
	...
	case 4:
		...
		if (use_wf_hardware) { BUFFERSOURCEHW(3, WF1); }
		else { BUFFERSOURCE(3, WF1); }
		while (height-- > 0)
		{
		    (*(lO[0].proc))(&lO[0]);
		    (*(lO[1].proc))(&lO[1]);
