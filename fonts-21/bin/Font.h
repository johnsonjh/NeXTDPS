...
typedef struct _NXFaceInfo {
	...
	unsigned int	usedInSheet:1;	/* has font been used in sheet? */
     /* There can be more than one page printed on a sheet of paper */
...
extern char *NXSystemFont;
extern char *NXBoldSystemFont;
...
+ initialize; 
+ newFont:(const char *)fontName size:(float )fontSize style:(int )fontStyle matrix:(float *)fontMatrix; 
